...
def run():
    with TestClient(Api.app) as c:
        ...
        show("test_generate_refuses_to_substitute_without_a_key")
        # Pressing "generate with DeepSeek" must give DeepSeek or an
        # error. Quietly returning a locally-built case is worse than
        # failing: it looks identical and the player cannot tell.
        from Deepseek import have_key
        r = c.post("/api/case/generate",
                   json={"n_suspects": 3, "difficulty": "normal"}).json()
        j = wait(c, r["job"], limit=240)
        if have_key():
            assert j["status"] == "done", j
            assert j["result"]["generated_by"].startswith("deepseek"), j["result"]
            print(f"  generated by {j['result']['generated_by']}")
        else:
            assert j["status"] == "failed", "it substituted silently"
            assert ".env" in j["error"], j["error"]
            print(f"  refused without a key: {j['error'].splitlines()[0]}")

        show("test_generate_runs_as_a_job")
        r = c.post("/api/case/generate",
                   json={"n_suspects": 3, "difficulty": "normal",
                         "allow_offline": True}).json()
        j = wait(c, r["job"], limit=240)
        assert j["status"] == "done", j
        res = j["result"]
        ...
        show("test_lab_follows_the_loaded_case")
        # The bug: Mcp_server hardcoded Case_Ashfield.json, so as soon as
        # you played a generated case the lab answered from the wrong
        # file and returned empty results.
        lib = c.get("/api/cases").json()["cases"]
        other = next((x for x in lib if x["case_id"] != "ashfield_001"), None)
        if other:
            c.post("/api/case/load",
                   json={"case_id": other["case_id"], "difficulty": "easy"})
            st = c.get("/api/state").json()
            sid = st["suspects"][0]["id"]
            r = c.post("/api/tool", json={"name": "movements_of",
                                          "args": {"suspect_id": sid}})
            assert r.status_code == 200, r.text
            moves = r.json()["result"]["movements"]
            assert moves, (f"lab returned nothing for {sid} in "
                           f"{other['case_id']} - it is serving the wrong case")
            print(f"  {other['case_id']}: movements_of({sid}) -> "
                  f"{len(moves)} entries, lab is on the right case")
        else:
            print("  SKIPPED - only one case in the library")

        show("test_deepseek_status_endpoint")
        d = c.get("/api/deepseek").json()
        assert "live" in d
        if d["live"]:
            print(f"  LIVE key {d['key_prefix']}... ping {d['ping']}")
        else:
            print(f"  offline: {d['reason'][:70]}")
        ...
        # the debrief has to actually coach, not just score
        assert v["grade"] in list("ABCDF"), v["grade"]
        assert v["the_solution"], "no solution explained"
        assert v["how_to_improve"], "no advice given"
        print(f"  grade {v['grade']}, {len(v['how_to_improve'])} pieces of "
              f"advice, solution explained")
        g = c.get("/api/state").json()["game"]
